...
### Next.js (learn-loop-studio)
- API route のエラー → route.ts のバリデーション、リポジトリ呼び出し、Supabase クエリを確認
- UI エラー → React コンポーネントの状態管理、hooks の依存配列、レンダリング条件を確認
- 認証エラー → lib/supabase/auth.ts、proxy.ts の CORS 設定を確認

### Flutter (learn_loop_app)
- クラッシュ → スタックトレースのフレームを辿り、null 安全性・型キャストを確認
- 状態管理エラー → Provider/Riverpod の状態更新フロー、非同期処理のライフサイクルを確認
- API 連携エラー → data/ のリポジトリ実装、リクエスト/レスポンスの型変換を確認
